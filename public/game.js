          ...
          for (const action in baseSprites) {
            for (const direction in baseSprites[action]) {
              ...
              if (!player.sprites[action][direction]) {
                ...
                player.sprites[action][direction] = recolorSprite(baseSpriteSheet, player.color);
              }
            }
          }
        }
...
ws.onmessage = event => {
  ...
  switch (data.type) {
    ...
    case 'map':
      ...
      break;
    case 'update':
      console.log('Update received, allSpritesLoaded:', allSpritesLoaded, 'players in update:', Object.keys(data.players));
      ...
      for (const id in serverPlayers) {
        ...
        if (existingPlayer) {
          ...
          newPlayers[id].sprites = { 'IDLE': {}, 'RUN': {}, 'ATTACK 1': {}, 'ATTACK 2': {} };
...
function draw() {
  ...
  for (const id in players) {
    ...
    const spriteSheet = player.sprites[player.action] && player.sprites[player.action][player.direction];
    if (spriteSheet) {
